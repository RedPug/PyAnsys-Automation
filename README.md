...
Create and activate a virtual Python environment (optional but recommended)

Install the Jupyter Notebook extension for VSCode

Open mapdl_solver.ipynb
- Create new Kernel
- May prompt to install an extension, do this
...
The cdb file contains all of the needed geometric, material, and static boundary properties to run a simulation. This solver creates the dynamic boundary condition (convection or temperature applied to a surface), so these should not be included in the cdb file.
...
5. Press `Solve` and a file will be generated at the specified location which contains the necessary information.   
    You may get an error message saying that there aren't any conditions. This is fine, just ignore it.
...
The number of processors to use can be changed at the top of `mapdl_solver.ipynb`

## Parameters
- `"model"`
    - The name of the cdb model to use. This only includes geometric and material properties, and named surface selections. **DO NOT HAVE ACTIVE BOUNDARY CONDITIONS WHEN EXPORTING**
...
You can either use `"trials"` or `"trial_input_folder"` to define trials, which describe the details of each simulation to perform. The solver will run all trials sequentially.
